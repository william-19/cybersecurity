...
class CryptoProject(object):
    ...
    def __init__(self):
        ...
        self.student_id = 'cwijaya3' #this is my Georgia Tech ID
    ...
    def mod_inverse(self, m, a):
        g, x, y = self.e_gcd(a, m)
        return x % m
    ...
    def recover_msg(self, N1, N2, N3, C1, C2, C3):
        ...
        n1_inverse = self.mod_inverse(N1, N2*N3)
        n2_inverse = self.mod_inverse(N2, N1*N3)
        n3_inverse = self.mod_inverse(N3, N2*N1)

        first = n1_inverse*N2*N3
        second = n2_inverse*N1*N3
        third = n3_inverse*N1*N2
        
        m_cube = (C1 * first + C2 * second + C3 * third) % (N1 * N2 * N3)
